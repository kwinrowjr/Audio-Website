...
echo "<h2>Albums are listed Alphabetically</h2>";
echo "<div id='boxtop'>";
...
echo"<div id='box1top'>";
	...
	echo "<table id=\"thetable\" cellspacing=\"0\" width=\"100%\">";
	...
	if ($rowcount > 0 ) {
		...
		while ($row = mysqli_fetch_assoc($query)) {
		   // printf ("%s (%s)\n", $row["Name"], $row["Code"]);
			//echo "<br>";
		$color = ($c++%2==1) ? 'bgcolor=#ffffff':  'bgcolor=#f1f1f1' ;
		/* echo "<tr   $color><td margin='7' HEIGHT='55' WIDTH='41'>"; */
		echo "<tr   $color> ";
		 
		 
		 /* <td margin='7' HEIGHT='55' WIDTH='41'>   */
		echo "<tr   $color><td class='align' margin='2' HEIGHT='55' WIDTH='1%'>";
		echo "<img style='height:50px; width:50px;' src='". $row['image'] ."' />"; 
		echo "</td><td class='align2' HEIGHT='55' WIDTH='50%'>";
		echo '<a href="musicplayer.php?album=' . $row['entry'] . '">
		' . $row['artist']. ' - ' . $row['title']. '</a><br />';
		echo"</td><td class='align' HEIGHT='auto' WIDTH='5%'>";
		echo $row['type'] ; 
		echo "</td></tr>";
