...
<?php
$value = $_GET['release'];
$year = addslashes($value);		
		$db = mysqli_connect("localhost","kwinrow_admin","northboi23", "kwinrow_storage")or die (mysqli_error());


	
// find out how many rows are in the table 
$sql = mysqli_query($db, "SELECT COUNT(*) FROM post_data WHERE year LIKE '%{$year}%'")or mysqli_error();
$sql2 = mysqli_query($db, "SELECT * FROM post_data WHERE year LIKE '%{$year}%'")or mysqli_error();
...
// get the info from the db 
	...
	if($rowcount = 0){
		...
		$query = mysqli_query($db, "SELECT * FROM post_data WHERE year LIKE '%{$year}%' ORDER BY year DESC $limit") or mysqli_error();
